...
server1="srv01.isp.nordsec.no"
username1="alexander"
blacklist1="/etc/fail2ban/ip.blocklist.list"

server2="db01.isp.nordsec.no"
username2="root"
...
# Now we send the full list back to server 1 again#
	scp $blocklist2 $username1'@'$server1:/home/alexander/ip.blocklist.list
exit;
"
# And merge the file again #
		mv /home/alexander/ip.blocklist.list /tmp/ip.blocklist.list1 && mv /etc/fail2ban/ip.blocklist.list /tmp/ip.blocklist.list2;
		cat /tmp/ip.blocklist.list1 /tmp/ip.blocklist.list2 > /etc/fail2ban/ip.blocklist.list;
